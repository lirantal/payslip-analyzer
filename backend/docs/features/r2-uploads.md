...
- `x-amz-meta-uploader-id` (anonymous upload: session ID)
- `x-amz-meta-uploader-type` (user vs anonymous)
- `x-amz-meta-file-id` (file record ID)
- `x-amz-meta-link-id` (upload link ID)

**Important**: When adding new metadata headers to your presigned URL signing, you MUST also update the R2 CORS policy to allow these headers. Failure to do so will result in CORS errors during the direct PUT upload to R2.
...
**Signed Headers Must Be Sent (Critical)**:
- **Problem**: Presigned URLs sign specific headers into the signature. If the frontend doesn't send ALL signed headers, R2 returns 403 Forbidden
- **Symptoms**: CORS error in browser console even though CORS is configured correctly; 403 on PUT request
- **Root Cause**: Backend was signing 7 headers (`Content-Type` + 6 `x-amz-meta-*`) but only returning 3 in the response; frontend was only sending `Content-Type`
- **Solution**: 
  1. Backend must return ALL signed headers in the presign response
  2. Frontend must send ALL headers from the response in the PUT request
- **Rule**: The set of headers signed = the set of headers returned = the set of headers sent

**CORS Configuration Fix (Critical)**:
- **Problem**: Using `"headers": ["*"]` wildcards in CORS policy caused 403 errors
- **Solution**: Specify exact headers in CORS config - every `x-amz-meta-*` header you sign must be listed
- **Why**: Cloudflare R2 doesn't support wildcard headers like AWS S3
- **Symptoms**: CORS error in browser console: "No 'Access-Control-Allow-Origin' header is present on the requested resource"
- **Diagnosis**: Check browser devtools → Network → preflight OPTIONS request; compare signed headers vs CORS policy

**Deferred File Record Creation (Critical)**:
- **Problem**: Creating file records at presign time causes orphaned records if R2 upload fails; retry attempts then show "duplicate file" errors
- **Solution**: File records are created at confirm time, not presign time
- **Flow**:
  1. `/presign` validates request, checks for duplicates by hash, generates presigned URL, returns metadata needed for confirm
  2. Frontend uploads directly to R2
  3. `/confirm` creates the file record in the database
- **Benefits**:
  - Failed uploads don't leave orphaned database records
  - Retries work correctly without false duplicate detection
  - Confirm endpoint is idempotent (same fileId returns success if already exists)
