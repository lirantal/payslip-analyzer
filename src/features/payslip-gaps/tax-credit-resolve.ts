import type { Insight, TaxCreditPointsField } from "../../types.js";

/**
 * Normalized y below this (top ~4.5% of image) is often logos, ref numbers, or unrelated zeros —
 * not the personal-header נקודות זיכוי cell. Model frequently mis-associates box_2d with the wrong "0".
 */
...
export function isNekudotZikuiInsight(insight: Insight): boolean {
  ...
  // Common header abbreviation: נ״ז (U+05E0 U+05F4 U+05D6) or ASCII quote variants
...
/** True if the box sits in the extreme top strip (likely not the tax-credit header cell). */
...
/**
 * Merges personal_header.tax_credit_points with a dedicated insights row (when the model splits them).
 * Fixes common failures: wrong "0" + spurious top-of-page box, or personal_header disagreeing with the insight row.
 */
